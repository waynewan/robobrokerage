...
XP_ACC_CASH_LBL = "//div[@class='funds-title'][text()='Available to trade']"
...
def wait_for_preview_accepted_or_error(driver, timeout=60):
	...
	while total_time > 0:
		...
		if len(elements) > 0 and len(elements[0].text) > 0:
			...
			return elements[0]
	raise TimeoutError("timeout waiting for order preview")


def page_status(driver):
	if len(driver.find_elements(By.XPATH, XP_PREVIEW_BTN)) > 0:
		return PAGE_STATUS_NEW
	if len(driver.find_elements(By.XPATH, XP_PLACE_ORDER_BTN)) > 0:
		return PAGE_STATUS_PREVIEW
	if len(driver.find_elements(By.XPATH, XP_NEW_ORDER_BTN)) > 0:
...
def fid_new_order_select_option(driver, list_name, option_name):
	...
	for ele in driver.find_elements(By.XPATH, XPATH_STR_FORM_BTN):
		...
		if lines[0].upper().startswith('WARNING:'):
			lines.pop(0)
		if list_name.lower() in lines[0].lower():
			drop_list_btn = ele
			ele.click()
			break
	if drop_list_btn is None:
		raise Exception("Cannot find dropbox name: " + list_name)
	# --
	# -- wait for options to appear (connection may be slow)
	# --
	drop_list = []
	for sltime in (0.3, 0.5, 1.0, 2.0):
		...
		if len(drop_list) > 0:
			break
	if len(drop_list) == 0:
		raise Exception("Cannot load dropbox name: " + list_name)
	# --
	...
	for ele in drop_list:
		...
		if len(lines) != 1:
			continue
		if option_name.lower() in lines[0].lower():
			option_selected_text = ele.text
			ele.click()
			return option_selected_text
	raise Exception("Cannot find option name: " + option_name)
...
def fid_new_order_enter_text(driver, box_label, inp_text):
	...
	for ele in driver.find_elements(By.XPATH, XPATH_STR_FORM_INP):
		if box_label.lower() in ele.text.lower():
			...
			return
	raise Exception("Cannot find input with label: " + box_label)
...
def select_account(driver, account):
	return retry(
		...
		retry=10, rtnEx=False, silent=True
	)
...
def refresh_current_market(driver):
	ele = wait_for_xpath(driver, XP_REFRESH_BTN)
	ele.click()
...
def get_current_market(driver, timeout=10):
	...
	# --
	refresh_current_market(driver)
	quote_panel = WebDriverWait(driver, timeout).until(
		EC.visibility_of_element_located((By.XPATH, XPATH_STR_FULL_ORD_QUOTE_PANEL))
	)
	# --
