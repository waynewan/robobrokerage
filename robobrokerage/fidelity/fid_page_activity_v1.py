...
XP_ACT_COLLAPSED = ".//div[@aria-label='show info'][@aria-expanded='false']"
# --
...
def raw_transactions(driver,incl_details=True):
	return raw_transactions_v20230920(driver,incl_details)

def formatted_transactions(raw_txns):
	return formatted_transactions_v20230920(raw_txns)

def raw_transactions_v20230920(driver,incl_details=True):
	# --
	if(incl_details):
		expand_all(driver)
	# --
	...
	for order in screen_row:
		...
		# --
		cells = order.find_elements(By.XPATH, "./div[@role='cell']")
		if(len(cells)<=3):
			# -- DEBUG --
			# header = "^".join([ cc.text for cc in cells if(cc is not None) ])
			# print("header:",header)
			continue
		order1['Date'] = cells[0].text
		order1['Description'] = cells[1].text
		order1['Symbol'] = None
		...
		order1['Amount'] = cells[2].text
		order1['Balance'] = cells[3].text
		order1['Details'] = ""
		order1['Price'] = ""
		order1['Shares'] = ""
...
def formatted_transactions_v20230920(raw_txns):
	# --
...
def raw_transactions_v0(driver,incl_details=True):
	# --
	if(incl_details):
		expand_all(driver)
	# --
...
def formatted_transactions_v0(raw_txns):
	# --
	# --
	# --
